...
//         s_FileRefDataCreateFromFile =
//             fileRefData.GetMethod("CreateFromFile", BindingFlags.Static | BindingFlags.Public);
//         s_PBXBuildFileDataCreateFromFile =
//             s_PBXBuildFileDat.GetMethod("CreateFromFile", BindingFlags.Static | BindingFlags.Public);
...
static class PBXProjectExtensions {
    ...
    public static void RemoveLocaleVariantFile(this PBXProject project, string groupName, string code, string path) {
        path = path.Replace('\\', '/');
        ...
        var res = (bool)s_GUIDListContains.Invoke(groupChildren, new object[] { fileRefsGuid });
        if (res) {
            s_GuidListRemove.Invoke(groupChildren, new[] { fileRefsGuid });
        }
    }
}
