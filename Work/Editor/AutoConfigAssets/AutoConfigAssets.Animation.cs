...
public class OptimizeAnimationClipTool
{
    ...
    private static bool SetFootEvent(AnimationOpt opt,KeyValuePair<string,int> p,string type)
    {
        bool set = false;
        Debug.Log($"opt.path.Contains{p.Key},{opt.path}");
        if (opt.path.Contains(p.Key, StringComparison.CurrentCultureIgnoreCase))
        {
            var events=AnimationUtility.GetAnimationEvents(opt.Clip);
            foreach (var evt in events)
            {
                if (evt.functionName == "ReceiveAnimEvent" && evt.stringParameter ==type)
                {
                    set = true;
                    var evtLst=events.ToList();
                    foreach (var _evt in evtLst)
                    {
                        if (_evt.stringParameter == type)
                        {
                            _evt.time = float.Parse((p.Value / 30f).ToString("0.000"));
                            break;
                        }
                    }
                    AnimationUtility.SetAnimationEvents(opt.Clip,evtLst.ToArray());
                    Debug.Log($"当前clip{opt.Clip}事件已存在,替换为最新配置参数 {type}");
                    break;
                }
            }
            if (!set)
            {
                set = true;
                var evtLst=events.ToList();
                evtLst.Add(new AnimationEvent()
                {
                    functionName = "ReceiveAnimEvent",
                    stringParameter = type,
                    time = float.Parse((p.Value/30f).ToString("0.000"))
                });
                AnimationUtility.SetAnimationEvents(opt.Clip,evtLst.ToArray());
                Debug.Log($"当前clip{opt.Clip}事件不存在,已设置 {type}");
            }
        }

        return set;
    }
    [MenuItem("Assets/Animation/设置角色行走或坐骑骑乘动画脚印事件")]
    public static void SetRoleAnimationFootEvent()
    {
        _AnimOptList = FindAnims();
       // Debug.Log($"{_AnimOptList.Count}");
        if (_AnimOptList.Count > 0)
        {
            foreach (var opt in _AnimOptList)
            {
               // Debug.Log($"{opt.path},{opt.path.Contains("walk")}");
                if (opt.path.Contains("walk")||opt.path.Contains("sprint") || opt.path.Contains("walk_ride"))
                {
                    //右脚
                    foreach (var p in AutoConfigAssets_AnimationConfig.rightFootEventsLst)
                    {
                        if (SetFootEvent(opt, p , "2"))
                        {
                            break;
                        }
                    }
                    //左脚
                    foreach (var p in AutoConfigAssets_AnimationConfig.leftFootEventsLst)
                    {
                        if (SetFootEvent(opt, p , "3"))
                        {
                            break;
                        }
                    }
