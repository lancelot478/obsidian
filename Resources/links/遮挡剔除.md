...
local function RefreshFadeObstructions(hits)  
    ...
    while index <= #hits do  
        ...
        if hideInfo == nil then  
            ...
            end            hideInfo.renderArr = renderArr  
...
function FadeObstructionManager.UpdateAction()  
    if not FadeObstructionManager.FadeOpen then  
        return    end    local camTra = BattleCamera.GetCameraTra()  
    if camTra == nil then  
        return    end    if MainInterfaceData.IsInHomeBuildTab() then--营地无需此功能  
        return  
    end    local deltaTime = GameMain:GetDeltaTime()  
